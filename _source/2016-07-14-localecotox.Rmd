...
require(knitr)
opts_chunk$set(fig.height = 6, fig.width = 8, message = FALSE, echo = TRUE, cache = FALSE)
require(methods)
```
...
We  get `r nrow(res)` entries, from different exposure times and endpoints.
...
Which reduces the data to `r nrow(res)` entries. 
...
We end up with `r nrow(res)` entries from `r length(unique(res$latin_name))` taxa.
...
We are left with data from `r nrow(res2)` taxa.
