...
Looking at the {% highlight r %}match_val column{% endhighlight %} we see that there has been no match (match_val == NA) for taxon *'xxxxxxxxx'*  and *'Baetis rodani'* has been matched approximately  (match_val = 0.1) with *'Baetis rhodani'*. The value in match_val indicates how good the guess for the species is.
...
![plot of chunk spear_plot](/figures/spear_plot.png) 
